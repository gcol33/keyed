...
Current CRAN version: **0.1.3**
Submitting to CRAN: **0.2.0**

## CRAN Policy

CRAN has **no explicit timeline** for removing deprecated functions.
The community convention is **at least 6 months** of soft-deprecation
before removal, longer for popular packages. The only hard rule is:
changes that break reverse dependencies must be coordinated with CRAN
...
- [Posit Community discussion](https://forum.posit.co/t/when-can-i-remove-a-deprecated-function-from-a-package/9707)
...
|----------|-------------|---------------|--------------|------------------|
| `commit_keyed()` | `stamp()` | 0.2.0 | Pending | TBD + 6 months |

## Timeline

### `commit_keyed()` → `stamp()`

- **0.1.0–0.1.3** (CRAN): `commit_keyed()` is the only snapshot function.
- **0.2.0** (internal): Soft-deprecated with `lifecycle::deprecate_warn()`.
  Ships to CRAN with **0.2.0**.
  Users see a warning once per session pointing to `stamp()`.
