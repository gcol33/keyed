...
1. **Legitimate transformations break keys** — Aggregation, cross-joins, reshaping
2. **Debugging is easier** — You see the result + warning, not just an error
3. **Pipelines don't halt unexpectedly** — You control when to enforce strictly
...
2. **Check assumptions at boundaries** — `assume_unique()`, `assume_no_na()`
